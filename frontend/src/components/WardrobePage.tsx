...
const WardrobePage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (isIntersecting && filteredItems.length > visibleItemsCount) {
      setVisibleItemsCount(prev => Math.min(prev + 12, filteredItems.length));
    }
  }, [isIntersecting]);
  ...
  }, [items, selectedCategory, debouncedSearchQuery, sortBy]);
